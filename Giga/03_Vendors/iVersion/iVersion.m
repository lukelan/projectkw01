...
static NSString *const updateAvailableTitleString = @"ギガニュースの新しいバージョンが入手可能です。今すぐバージョン %@ にアップデートしてください。";
...
{
    return _inThisVersionTitle ?: [self localizedStringForKey:iVersionInThisVersionTitleKey withDefault:@"新しいバージョン"];
}
    ...
    if (self.checkingForNewVersion)
    {
        ...
        if (details)
        {
            ...
            if (showDetails && !self.visibleRemoteAlert)
            {
                ...
                if (!self.groupNotesByVersion)
                {
                    title = [NSString stringWithFormat:updateAvailableTitleString, mostRecentVersion];
                }
